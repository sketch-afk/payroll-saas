...
export async function PUT(req, { params }) {
  try {
    ...
    // SCENARIO 1: Quick Status Toggle (Deactivate / Reactivate)
    // If the frontend only sends the "status" field, we just update the status.
    ...
    // SCENARIO 2: Full Form Edit (Editing an employee's details)
    const {
    ...
    await query(
      `UPDATE employees
       SET first_name = :fn, last_name = :ln, email = :email, phone = :phone,
           hire_date = TO_DATE(:hd, 'YYYY-MM-DD'), job_title = :jt, dept_id = :dept,
           status = :status
    ...
    await query(
      `UPDATE salary_structures
       ...
       WHERE emp_id = :id AND company_id = :cid AND is_current = 1`,
      {
...
export async function DELETE(req, { params }) {
  try {
    ...
    // To perform a hard delete, we first need to delete their salary structure 
    // to prevent Oracle Foreign Key constraints from blocking us.
    await query(
    ...
    // If they have attendance or payroll records, Oracle will block the deletion (ORA-02292)
