...
export async function POST(req) {
  try {
    ...
    if (!emp_id || !from_date || !to_date) {
      return NextResponse.json({ error: 'emp_id, from_date, and to_date are required' }, { status: 400 });
    }
    ...
    await query(
      `INSERT INTO leaves (company_id, emp_id, leave_type, from_date, to_date, reason, status)
       VALUES (:cid, :eid, :type, TO_DATE(:fd, 'YYYY-MM-DD'), TO_DATE(:td, 'YYYY-MM-DD'), :reason, 'PENDING')`,
      { 
        cid, 
        eid: emp_id, 
...
export async function PUT(req) {
  try {
    ...
    return NextResponse.json({ message: `Leave ${status.toLowerCase()} successfully` });
  } catch (e) {
    if (e.message === 'UNAUTHORIZED') return unauthorized();
    return serverError(e);
  }
