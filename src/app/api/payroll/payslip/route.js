  ...
  try {
    ...
    const data = await queryOne(
      `SELECT
         ...
         p.status,
         p.processed_at,
         NVL(a.present, 0) AS present_days,
         NVL(a.absent, 0) AS absent_days,
         NVL(a.half_day, 0) AS half_day_count,
         NVL(a.leave_count, 0) AS leave_days
       FROM payroll p
    ...
    if (!data) {
      return NextResponse.json(
        { error: "Payslip details not found" },
        { status: 404 },
      );
    }
