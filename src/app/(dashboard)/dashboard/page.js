...
export default function DashboardPage() {
  ...
  // 1. FIXED: Add an isMounted flag to prevent Next.js Hydration Errors
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => {
    setIsMounted(true); // Component has safely mounted in the browser
  ...
  // 2. FIXED: Memoize the random data so it only calculates once on the client, 
  // preventing it from scrambling every time the component re-renders.
  const barData = useMemo(() => {
    // If the API hasn't loaded or didn't send trend data, show an empty chart
    if (!stats?.payrollTrend || stats.payrollTrend.length === 0) {
      return [];
    }

    // Map the backend data into the format Recharts expects
    // Expecting backend to send: [{ pay_month: 1, total_gross: 500000, total_net: 400000 }]
    return stats.payrollTrend.map(pt => ({
      month: MONTHS[(pt.PAY_MONTH || pt.pay_month) - 1], // Convert 1 -> 'Jan', 2 -> 'Feb'
      gross: Number(pt.TOTAL_GROSS || pt.total_gross || 0),
      net:   Number(pt.TOTAL_NET || pt.total_net || 0),
    }));
  }, [stats?.payrollTrend]);
  ...
  // Now it's safe to use standard browser APIs
  const now = new Date();
