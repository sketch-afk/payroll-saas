...
export default function AttendancePage() {
  ...
  const [month, setMonth]           = useState(1); // Placeholder until mount
  const [year, setYear]             = useState(2026); // Placeholder until mount
  const [selectedDate, setSelectedDate] = useState(''); // Placeholder until mount
  const [bulkStatus, setBulkStatus]     = useState('PRESENT');

  // Map emp_id -> status for the selected date
  const [dayMap, setDayMap] = useState({});

  // 1. FIXED: Set the date based on local browser time after mounting to prevent Hydration errors
  useEffect(() => {
    const now = new Date();
    setMonth(now.getMonth() + 1);
    setYear(now.getFullYear());
    
    // 'en-CA' natively outputs local time as YYYY-MM-DD, avoiding the UTC timezone shift!
  ...
  useEffect(() => {
    ...
    fetch(`/api/attendance?month=${month}&year=${year}`)
      ...
      .then(d => {
        ...
        // Build dayMap for selected date
        const map = {};
        (d.data || []).forEach(r => {
          // 2. FIXED: Safely parse database dates to local YYYY-MM-DD format
  ...
  // Prevent rendering until safely mounted in the browser
